...
namespace raspichu.inspector_enhancements.editor
{
    public class VixenEnhancements : EditorWindow
    {
        ...
        private static Dictionary<int, GameObject> persistentSelectedGameObject = new Dictionary<int, GameObject>(); // Dictionary to store persistently selected transforms
        ...
        public static void OnInspectorGUI(UnityEditor.Editor __instance)
        {
            ...
            bool hasVixenControl = persistentSelectedGameObject.Values.All(go => go.GetComponent<VixenControl>() != null);
            if (persistentSelectedGameObject.Count == 0 || hasVixenControl) return;
        ...
        private static void CreateVixenObjects(bool toggleOn)
        {
            ...
            if (persistentSelectedGameObject.Count == 0) return;
            ...
            if (forEach)
            {
                for (int i = 0; i < selectedGameObjects.Count; i++)
                {
                    ...
                    lastSelectedObject = selectedGameObjects.Count > 0 ? selectedGameObjects.Last() : lastSelectedObject;
                    GameObject newObject = createVixenFrom(name, new GameObject[] { selectedGameObjects[i] }, toggleOn, lastSelectedObject.transform);
            ...
            {
                string persistentSelectedGameObjectNames = GetNameFrom(selectedGameObjects.ToArray());
                string name = $"{persistentSelectedGameObjectNames} - {toggleStateName}";

                GameObject newObject = createVixenFrom(name, selectedGameObjects.ToArray(), toggleOn, lastSelectedObject.transform);
        ...
        private static GameObject createVixenFrom(string name, GameObject[] toggleObjects, bool toggleOn, Transform position)
        {
